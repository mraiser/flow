...
impl<'a> DataObject<'a> {
  pub fn new(env:&'a FlowEnv) -> DataObject<'a> {
  ...
  pub fn get(data_ref: usize, env:&'a FlowEnv) -> DataObject<'a> {
  ...
  pub fn from_json(value:Value, env:&FlowEnv) -> DataObject {
    ...
    for (key, val) in value.as_object().unwrap().iter() {
      ...
      else if val.is_object() { o.put_object(key, DataObject::from_json(val.to_owned(), env)); }
      else if val.is_array() { o.put_list(key, DataArray::from_json(val.to_owned(), env)); }      
  ...
  pub fn shallow_copy(&self) -> DataObject {
    let mut o = DataObject::new(self.env);
  ...
  pub fn deep_copy(&'a self) -> DataObject<'a> {
    let mut o:DataObject<'a> = DataObject::new(self.env);
  ...
  pub fn get_object(&self, key:&str) -> DataObject<'a> {
    self.get_property(key).object(self.env)
  }
  
  pub fn get_array(&self, key:&str) -> DataArray<'a> {
    self.get_property(key).array(self.env)
  }
...
impl<'a> IntoIterator for DataObject<'a> {
...
impl<'a> fmt::Debug for DataObject<'a> {
